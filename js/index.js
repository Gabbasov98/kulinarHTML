...
function sliderNew() {
    var swiper = new Swiper('.new.new1 .swiper-container', {
        speed: 800,
        slidesPerView: 5,
        spaceBetween: 0,
        navigation: {
            nextEl: '.new.new1 .swiper-button-next',
            prevEl: '.new.new1 .swiper-button-prev',
        },
...
$(document).ready(function() {
    ...
    $(".question-modal__phone input").click(function() {
        $(".question-modal__phone input").setCursorPosition(3)
    }).mask("+7 (999) 999-99-99")
    ...
    if ($(".catalog-menu__show").hasClass("catalog-menu__show--active")) {
        $(".catalog-menu__show--active").siblings(".catalog-menu__hidden").slideToggle(400);
    }
    ...
    $(document).mouseup(function(e) { // событие клика по веб-документу
        var div = $(".catalog-list__param-show--active").parent(".catalog-list__param-item"); // тут указываем ID элемента
        if (!div.is(e.target) // если клик был не по нашему блоку
            &&
            div.has(e.target).length === 0) { // и не по его дочерним элементам
            // div.hide(); // скрываем его
            div.children(".catalog-list__param-hidden").hide();
            div.children(".catalog-list__param-show").removeClass("catalog-list__param-show--active")
        }
    });
    ...
    // регулирование количества товаров

    $('.cart__item-minus').click(function() {
        var a = $(this).closest('.cart__item-control').find('input').val();
        if (a > 1) {
            var b = +a - 1;
            $(this).closest('.cart__item-control').find('input').val(b);
        } else {
            $(this).closest('.cart__item-control').find('input').val(a);
        }
    });
    ...
    $('.catalog-detail__minus').click(function() {
        var a = $(this).closest('.catalog-detail__control').find('input').val();
        if (a > 1) {
            var b = +a - 1;
            $(this).closest('.catalog-detail__control').find('input').val(b);
        } else {
            $(this).closest('.catalog-detail__control').find('input').val(a);
        }
    });
    $('.catalog-detail__plus').click(function() {
        var a = $(this).closest('.catalog-detail__control').find('input').val();
        var b = +a + 1;
        $(this).closest('.catalog-detail__control').find('input').val(b);
    });
    ...
    $('.catalog-list__param-inner').mCustomScrollbar({
        axis: 'y',
    });
    $('.header__menu-inner').mCustomScrollbar({
        axis: 'y',
    });
